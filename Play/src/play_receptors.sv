...
module play_receptors (
  ...
  localparam int TEXT_TOP_Y = 342;  // 評語字基準 y（框下）
  ...
  function automatic logic glyph_at(input logic [7:0] ch, input int px, input int py);
    ...
    begin
      glyph_at = 1'b0;
      if (px >= 0 && px < CHAR_PIX_W && py >= 0 && py < CHAR_H && ch >= "A" && ch <= "Z") begin
        offset   = (ch - "A") * CHAR_W;
  ...
  always_comb begin
    in_lane = 1'b0;
    lane = 2'd0;
    if (vga_x >= 10'd131 && vga_x < 10'd225) begin
      lane = 2'd0;
      in_lane = 1'b1;
    end else if (vga_x >= 10'd226 && vga_x < 10'd320) begin
      lane = 2'd1;
      in_lane = 1'b1;
    end else if (vga_x >= 10'd321 && vga_x < 10'd415) begin
      lane = 2'd2;
      in_lane = 1'b1;
    end else if (vga_x >= 10'd416 && vga_x < 10'd510) begin
      lane = 2'd3;
      in_lane = 1'b1;
    end
  end
  ...
  always_comb begin
    ...
    if (in_lane) begin
      ...
      top_bit   = 1'b0;
      valid_bit = 1'b0;
      if (vga_x >= tl && vga_x < tl + TOP_W && vga_y >= top_y && vga_y < top_y + TOP_H) begin
        dx_t = int'(vga_x) - tl;
        dy_t = int'(vga_y) - top_y;
        if (show_valid) valid_bit = rom_valid[dy_t[4:0]][TOP_W-1-dx_t];
        else top_bit = rom_top[dy_t[4:0]][TOP_W-1-dx_t];
      end
